...
output:
  html_document:
    theme: null # Disable R Markdown's default theme
    css: 'assets/css/style.css' # Link to the Cayman theme's main CSS
    highlight: 'textmate' # Choosen highlight style
---
...
* **Note on paths**: Use forward slashes (`/`) for paths, as R interprets them consistently across operating systems. Windows paths with backslashes (`\`) need to be escaped (e.g., `C:\\Users\\...`) or converted to forward slashes.
...
| :--- | :--- |
| **`setwd()`** | **Avoid using it in scripts**, especially with *absolute paths* (e.g., `C:/Users/MyName/...`). An absolute path will not work on someone else's computer. Use setwd() it in the console instead. |
