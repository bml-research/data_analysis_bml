...
length_protein <- c(546, 500, 1545, 1130, 632)

my_tibble <- tibble(gene, chromosome, length_protein)
...
Reading the information on read_csv2 learns us that the default setting for headers is TRUE and that R will read (by default) the first row as a header. We can ommit this problem by setting the col_names parameter to FALSE:
...
Often TSV-files are used. These are files where the values are separated by a tab. They can be imported using `read_tsv`.
...
It also gives you the opportunity to first do some analysis in Excel and continue in R, for example if the dataset appears to be to big to be handled by Excel.
