...
long_dino <- c("Dongyangosaurus", "Elaltitan", "Epachthosaurus", "Udelartitan",
               "Volgatitan", "Zhuchengtitan")
# Filter the data set using this vector.
df_long_dino <- sel_data %>%
  filter(`Scientific name`%in%long_dino)
formatted_table(df_long_dino)
# Create the pie chart based on the weight of these dinosaurs.
dino_pie <- ggplot(df_long_dino, aes(x = "", y = `Weight (kg)`, 
...
For the following plots we will use the [Climate disease dataset](./files_13_data_visualization_exercises/add_exercises/Gclimate_disease_dataset.csv).
```{r}
