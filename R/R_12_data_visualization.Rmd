...
- [file01_anage_data.csv](./files_12_data_visualization/file01_dataset_ zoo_big_cat_feeding.csv)  
...
barplot(barplot_data$samples_processed,
        ...
        ylab = "number of samples processed")
```
...
p <- barplot_data %>%
  ...
  labs(title="Lab samples processed in the week", y = "Number of samples processed") 
p
```
...
A stacked column chart should only be used when the individual values combine to form a meaningful total.
For example, it is not meaningful to plot birth weight and adult weight for different cat species in a stacked column chart. To demonstrate a meaningful use of a stacked column chart we will use another (fictional) dataset on the average feeding data for big cats in a zoo. In this chart, each column represents a species, and the segments within each column show the contribution of different food components (such as meat, bone, and supplements) to the total daily diet.  

```{r}
cat_diet <- read_csv("./files_12_data_visualization/file01_dataset_ zoo_big_cat_feeding.csv")
formatted_table(cat_diet)
```

Before we create the graph, we need to re-arrange the data in tidy format:

```{r}
cat_diet_tidy <- cat_diet %>%
  select(
    Species,
    Meat = Meat_kg_per_day_per_animal,
    Bone = Bone_kg_per_day_per_animal,
    Supplements = Supplements_kg_per_day_per_animal) %>%
  pivot_longer(
    cols = c(Meat, Bone, Supplements),
    names_to = "Component",
    values_to = "Value"
  ) 

formatted_table(cat_diet_tidy)
  
```

And now we can create the stacked barplot:  

```{r}
p <- cat_diet_tidy %>%
  ggplot(aes(x = `Species`, y = `Value`, fill = `Component`)) +
  geom_bar(stat = "identity", position="stack") +
  labs(title="Average contribution of food components to daily diet of big cats") +
  theme(axis.text.x = element_text(angle = 45, hjust=1))
p
```
\
...
If you want to change the order of groups you will need to change the column from character to a factor as factors do support levels:
...
The same can be done for the stacked barchart:

```{r}
cat_diet_tidy_ordered <- cat_diet_tidy %>%
  mutate(Component = factor(Component, levels = c("Supplements", "Bone", "Meat")))
formatted_table(cat_diet_tidy_ordered)
```


```{r}
p <- cat_diet_tidy_ordered %>%
  ggplot(aes(x = `Species`, y = `Value`, fill = `Component`)) +
  geom_bar(stat = "identity", position="stack") +
  labs(title="Average contribution of food components to daily diet of big cats") +
  theme(axis.text.x = element_text(angle = 45, hjust=1))
...
Like Excel, Pie charts are supported in the R package ggplot2. As an example, we use our previous (fictional) dataset to create a pie chart of the average daily diet of tigers in a zoo:  
...
p <- cat_diet_tidy_ordered %>%
  ...
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0) +
  labs(title="Average contribution of food components to the daily diet of tigers") +
  theme_void() # remove background, grid, numeric labels
p
```
