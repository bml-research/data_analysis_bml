...
Like the example in Excel, first a column chart will be created with the same data. The example below will show you the code to make a column chart in base R. To disregard `NA` values, the `na.rm = ` argument is set to `TRUE`:
...
barplot(summ_data$`Birth weight (g)`, 
        ...
        cex.names = 0.5,
        col = "steelblue",
        main = "Adult weight for different cat species")
```
...
Also here it is clear that the Bobcat has the highest maximum life span.
...
tidy_summ_data <- summ_data %>%
  gather(`Birth weight (g)`, `Adult weight (g)`, key = "Adult weight (g)", value = "Birth weight (g)")
...
```{r}
p <- ggplot(data = tidy_summ_data, aes(x = `Common name`, y = `Birth weight (g)`, fill = `Adult weight (g)`)) +
...
If you want to change the order of groups you will need to change the column `Adult weight (g)` from character to a factor as factors do support levels:
And we can change the order of levels accordingly:

```{r}
tidy_summ_data$`Adult weight (g)` <- factor(tidy_summ_data$`Adult weight (g)`, levels = c("Birth weight (g)", "Adult weight (g)"))
levels(tidy_summ_data$`Adult weight (g)`)
```
...
The same holds for the stacked and the Percent stacked barchart (only the first example is shown below):
...
```{r}
p <- ggplot(tidy_summ_data[16:25,], aes(x = "", y = `Birth weight (g)`, fill = `Common name`)) +
...
artiod_df[, 4:6] <- lapply(artiod_df[, 4:6], function(x) as.numeric(as.character(x)))
artiod_df <- na.omit(artiod_df)
...
```{r}
p <- ggplot(tidy_artiod, aes(x = `Family`, y = `Birth weight (g)`, fill = `Adult weight (g)`)) + 
  geom_boxplot() +
  scale_y_log10() +
...
```{r}
tidy_artiod$`Adult weight (g)` <- factor(tidy_artiod$`Adult weight (g)`, levels = c("Birth weight (g)", "Adult weight (g)"))
levels(tidy_artiod$`Adult weight (g)`)
```
...
As an example, we take the same (fictive data) about the OB/OB and DB/DB mice (compared to the wildtype) and the gain of weight in these mice during a period of time as was used in the Excel section.
...
tidy_mouse_weight <- mouse_weight %>%
  gather(`Wild-type`, `OB/OB`, `DB/DB`, key = "Mouse", value = "Weight (kg)") 
...
```{r}
p <- ggplot(data = tidy_mouse_weight, aes(x = Date, y = `Weight (kg)`, group = `Mouse`)) +
  geom_line() +
...
X-axis: Analysis time (minutes).<br>
Y-axis: Protein concentration (mg/mL).<br> 
Bubble size: Represents the number of assays per day in the laboratory.  
