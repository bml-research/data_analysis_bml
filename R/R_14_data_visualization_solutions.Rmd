...
barplot(df_ex2$`Log2 Total Global Average GHG Emissions per kg`, 
        ...
        ylim = c(0, max(df_ex2$`Log2 Total Global Average GHG Emissions per kg` + 4)), 
...
Highest: Oils and Fats and Vegatable Oils
...
For this exercise, we will use the dataset from exercise 7 again. This time, to create a `pivot table and plot` (note that there are no real pivot tables and plots in R).
