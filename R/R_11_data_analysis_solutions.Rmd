...
Perform the following calculations on this dataframe and display them as a table:  
...
Ah, nice! The summary already gives the mean and median for these proteins.
...
formatted_table(df %>%
                  ...
                  summarize("St.Dev PKCA" = round(sd(PKCA_N, na.rm = T), 3),
                            "Mean PKCA" = round(mean(PKCA_N, na.rm = T), 3), # just to check
                            "Median PKCA" = round(median(PKCA_N, na.rm = T), 3), # just to check
                            "SD.Dev RRP1" = round(sd(RRP1_N), 3),
                            "Mean RRP1" = round(mean(RRP1_N, na.rm = T), 3), # just to check
                            "Median RRP1" = round(median(RRP1_N, na.rm = T), 3), # just to check
                            "SD.Dev BRAF" = round(sd(BRAF_N, na.rm = T), 3),
                            "Mean BRAF" = round(mean(BRAF_N, na.rm = T), 3), # just to check
                            "Median BRAF" = round(median(BRAF_N, na.rm = T), 3), # just to check
                            "SD.Dev JNK" = round(sd(JNK_N, na.rm = T), 3),
                            "Mean JNK" = round(mean(JNK_N, na.rm = T), 3), # just to check
                            "Median JNK" = round(median(JNK_N, na.rm = T), 3) # just to check
                            ))
```
