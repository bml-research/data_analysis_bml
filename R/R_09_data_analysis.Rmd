...
- [R: Data Analysis](#r-data-analysis)
  ...
  - [Select rows by index](#select-rows-by-index)
  - [Select column by index](#select-column-by-index)
  - [Select column by name](#select-column-by-name)
  - [Select item by index](#select-item-by-index)
  - [Analysis with Tidyverse functions](#analysis-with-tidyverse-functions)
  - [Slicing with slice](#slicing-with-slice)
  ...
  - [Summarize each](#summarize-each)
    - [Summarize and across](#summarize-and-across)
...
As an example of a small data set we will use the build-in Puromycin data set.  

```{r}
Puromycin <- tibble(Puromycin)
formatted_table(head(Puromycin))
```


As an example of a larger data set, we will be using the same data set as used in the Excel section:
...
```{r}
formatted_table(Puromycin[2, ]) #select second row
```



Or for the larger data frame:
...
```{r}
formatted_table(head(Puromycin[, 2])) #select second column
```
...
```{r}
head(Puromycin$state) #select the state
```
...
For the Puromycin data set:

```{r}
formatted_table(Puromycin[2, 3]) #select second row and third column
```


And the larger data set:
...
Using `slice_max` you can get the row with a max value (for example the max rate) from the puromycin data set:  
...
Using `slice_max` you can get the row with a max value (for example the max Alcohol level) from the larger data set:  
...
Filter rows on conc 0.11 in Puromycin data set:
...
Filter rows on all patients of age 65 on the larger data set:
...
Concentration higher than 0.11 in Puromycin:
...
State is untreated and higher concentrations than 0.11 in Puromycin data set:

```{r}
Puromycin %>%
  filter(state == "untreated" & conc > 0.11) %>%
  slice_head(n = 6) %>%
  formatted_table()
```

Current smoker (condition 1) AND normal blood oxygen levels (condition 2) in larger data set:
...
NOT a untreated sample AND conc higer than 0.11 in Puromycin data set:
...
Conc lower than 0.11 OR higher than 0.56 in Puromycin data set:

```{r}

Puromycin %>%
  filter(conc < 0.11 | conc > 0.56) %>%
  slice_head(n = 6) %>%
  formatted_table()
```

Normal blood oxygen levels OR very low blood oxygen levels in the larger data set:
...
Arrange on rate **ascending** (from low to high values) in Puromycin data set:
...
Arrange on blood oxygen levels **ascending** (from low to high values) in larger data set:
...
Here is an example of sorting on state first (alphabetically from a to z) and then sorting on rate in the Puromycin data set: 
...
Here is an example of sorting on age first and then sorting on blood oxygen levels in the larger data set: 
...
Here is an example of sorting on state first (**descending** alphabetically from z to a) and then sorting on **descending** rate in the Puromycin data set:

```{r}
Puromycin %>%
  arrange(desc(state), desc(rate)) %>%
  slice_head(n = 6) %>%
  formatted_table()
```
...
For Puromycin select conc and rate:
...
And for the larger data set columns Age, Smoking_Status, BloodOxygenLevel, HeartRate:
...
Or select all columns except the state in the Puromycin data set:
...
Or select all columns except the blood oxygen levels in the larger data set:
...
Or select all column names that are numbers in the Puromycin data set:
...
Or select besides the age, all column names that end with "Level" in the larger data set:
...
Or the larger data set:

```{r}
summary(df)
```

As this output can be quit overwhelming you may select for the data that you are interested in:


For example the rate in the Puromycin data set:

```{r}
summary(Puromycin$rate)
```

For example blood oxygen levels for the larger data set:
...
Puromycin %>%
  group_by(state) %>%
  summarize(average_conc = mean(conc), average_rate = mean(rate)) %>%
  formatted_table()
```
...
Puromycin %>%
  group_by(state) %>%
  summarize(average_conc = mean(conc), sd_conc = sd(conc), average_rate = mean(rate), sd_rate = sd(rate)) %>%
  formatted_table()
```
...
Puromycin %>%
  group_by(state) %>%
  summarize(average_conc = round(mean(conc), digits = 1), 
            sd_conc = round(sd(conc), digits = 1),
            average_rate = round(mean(rate), digits = 1), 
            sd_rate = round(sd(rate), digits = 1)) %>%
  formatted_table()
```
