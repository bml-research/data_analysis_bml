...
But what we will use is the data frame. See it as a bunch of column stitched together:
...
So in the above code the vectors assigned to the variable names my_nums and my_dec are transferred as arguments in the data.frame function. This function returns a data frame object that is assigned to the my_df variable name.
...
Tibbles look nice in markdown documents but look very basic in html documents.
...
But I prefer the Twitter Bootstrap theme with striped rows that light up when you hover over it at that scales nicely on small screens like phones (you will get a horizontal slider when horizontal space is limited):
...
This also shows the power of R over Excel: several repetitive actions can be collected in a function so you can perform the same action on a different dataset, while in Excel you will have to do all the actions by hand with each new dataset.
---
