...
- [R: Data Cleaning](#r-data-cleaning)
  ...
  - [Changing a columns data type](#changing-a-columns-data-type)
  - [Deal with missing data from csv files](#deal-with-missing-data-from-csv-files)
  - [Deal with missing data from Excel files](#deal-with-missing-data-from-excel-files)
  - [Replace data](#replace-data)
    - [Adding a row in a tibble](#adding-a-row-in-a-tibble)
    - [Replacing a whole row:](#replacing-a-whole-row)
    - [Replacing data in a whole column:](#replacing-data-in-a-whole-column)
    - [Replacing data in entire data frame:](#replacing-data-in-entire-data-frame)
    - [Making data tidy](#making-data-tidy)
    - [Transpose data](#transpose-data)
    - [Separate a column into two columns](#separate-a-column-into-two-columns)
...
So on this page there are some examples of data cleaning.
But first we will explain a very useful Tidyverse operator, the `%>%` symbol.  
...
>You can use `pivot_wider()` and `pivot_longer()` to transpose data in the style of tidyverse but we find it more difficult to understand.
>Hence, we will use base R for this.
